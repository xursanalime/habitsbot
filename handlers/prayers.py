...
async def save_prayers(message: Message, state: FSMContext, db):
    ...
    try:
        ...
        isha_time = datetime.strptime(message.text, "%H:%M").time()
         ...
         await message.answer("❌ Vaqt formatida xatolik yuz berdi. Iltimos qaytadan urining.")
         await state.clear()
         return
    ...
    try:
        ...
        print(f"Prayer save error: {e}")
        await message.answer("❌ Namoz vaqtlarini saqlashda xatolik yuz berdi.")
...
async def prayer_done(callback: CallbackQuery, db):
    ...
    # To'g'ri vaqt zonasi orqali kunni olish
    ...
    try:
        ...
        print(f"Prayer done callback error: {e}")
